...
if __name__ == '__main__':
    train_env_num = 32
    test_env_num = 64
    envs = DummyVectorEnv([lambda: gym.make('ENVTEST-v3', is_random=False, is_test=False) for _ in range(train_env_num)])
    tmp = [lambda: gym.make('ENVTEST-v3', is_random=False, is_test=True, needs_print=True)]
    test_env = DummyVectorEnv(
        tmp + [lambda: gym.make('ENVTEST-v3', is_random=False, is_test=True) for _ in range(1, test_env_num)])

    model = HAN()

    actor = Actors(model, pointer_type="attn", no_pointer=True)
