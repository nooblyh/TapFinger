...
if __name__ == '__main__':
    ...
    hetero = False
    envs = DummyVectorEnv(
        [lambda: gym.make('ENVTEST-v3', is_random=False, is_test=False, hetero=hetero) for _ in range(train_env_num)])
    tmp = [lambda: gym.make('ENVTEST-v3', is_random=False, is_test=True, needs_print=True, hetero=hetero)]
    test_env = DummyVectorEnv(
        tmp + [lambda: gym.make('ENVTEST-v3', is_random=False, is_test=True, hetero=hetero) for _ in
               range(1, test_env_num)])

    model = HAN()

    actor = Actors(model, pointer_type="attn")
    ...
    train_collector = Collector(policy, envs,
                                tianshou.data.VectorReplayBuffer(total_size=64000, buffer_num=train_env_num))
