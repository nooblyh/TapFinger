...
if __name__ == '__main__':
    train_env_num = 32
    test_env_num = 64
    envs = DummyVectorEnv([lambda: gym.make('ENVTEST-v3', is_random=False, is_test=False, gnn_state=False) for _ in range(train_env_num)])
    tmp = [lambda: gym.make('ENVTEST-v3', is_random=False, is_test=True, needs_print=True, gnn_state=False)]
    test_env = DummyVectorEnv(
        tmp + [lambda: gym.make('ENVTEST-v3', is_random=False, is_test=True, gnn_state=False) for _ in range(1, test_env_num)])

    model = ToyModel(config.stacking_dim, config.hidden_dim)

    actor = Actors(model, pointer_type="attn", no_GNN=True)
    critic = Critics(model, preprocess_net_output_dim=config.hidden_dim, device=config.device, no_GNN=True)
    ...
    result = tianshou.trainer.onpolicy_trainer(
        ...
        max_epoch=400, step_per_epoch=200000, repeat_per_collect=2, episode_per_collect=32, episode_per_test=64,
